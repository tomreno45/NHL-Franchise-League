...
export const api = {
  ...
  subscribePush: (subscription) => send("POST", "/push/subscribe", subscription),
  unsubscribePush: (endpoint) => send("POST", "/push/unsubscribe", { endpoint }),
  sendCommissionerPush: (title, message) => send("POST", "/commissioner/push", { title, message }),
  ...
  autoSetLineup: (teamId) => send("POST", "/lineup/auto-set", { teamId }),
  getRosterChanges: () => get("/commissioner/roster-changes"),
  getRosterMoveSync: () => get("/commissioner/roster-moves"),
  clearRosterMoveSync: (playerIds) => send("POST", "/commissioner/roster-moves/clear", { playerIds }),
  ...
  adminGetAccounts: () => get("/admin/accounts"),
  adminCreateAccount: (payload) => send("POST", "/admin/accounts", payload),
  adminResetPassword: (accountId, newPassword) => send("POST", `/admin/accounts/${accountId}/reset-password`, { newPassword }),
  adminDeleteAccount: (accountId) => send("DELETE", `/admin/accounts/${accountId}`, {}),
  adminGetTeams: (leagueSlug) => get(`/admin/teams/${leagueSlug}`),
  adminAddMembership: (accountId, payload) => send("POST", `/admin/accounts/${accountId}/memberships`, payload),
  adminRemoveMembership: (membershipId) => send("DELETE", `/admin/memberships/${membershipId}`, {}),
